...
class Mage_Core_Model_Design_Package
{
    ...
    public function getMergedJsUrl($files)
    {
        ...
        foreach ($files as $file) {
            array_push($filesHash,hash_file('md5', $file));
        }

        $targetFilename = md5(implode(',', $filesHash)) . '.js';
    ...
    public function getMergedCssUrl($files)
    {
        $fileHash = array();
        foreach ($files as $file) {
            array_push($fileHash,hash_file('md5', $file));
        }
        // secure or unsecure
        ...
        $targetFilename = md5(implode(',', $fileHash)) . '.css';
